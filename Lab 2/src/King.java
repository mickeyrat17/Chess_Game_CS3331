...
public class King {
	...
	private char piece_name = 'k';
	private char color;
	private int column;
	private int row;
	
	public King(char color, int column, int row) {
		this.color = color;
		this.column = column;
		this.row = row;
	}
	...
	public  boolean move_logic(int move_column, int move_row) {
		...
		if(number_of_rows_moved > 1 ) {
			return !move_is_logical;
		}
		
		int number_of_columns_moved = Math.abs(get_column() - move_column);
		if(number_of_columns_moved > 1) {
			return !move_is_logical;
		}
		
		//case where piece doesn't move
		if(number_of_columns_moved == 0 && number_of_rows_moved ==0 ) {
			return !move_is_logical;
		}
